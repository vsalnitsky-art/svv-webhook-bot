...
class MarketAnalyzer:
    def __init__(self):
        ...
        # Запускаємо фоновий потік для оновлення статусів зон
        ...
        try:
            ...
            for i, t in enumerate(tickers):
                ...
                try:
                    ...
                    for sg in sigs:
                        # 1. Результат для таблиці сканера (миттєвий)
                        ...
                        # 2. Додавання в Watchlist (якщо монети там ще немає)
                ...
                for item in watchlist:
                    ...
                    try:
                        ...
                        if df is not None:
                            ...
                            # Видаляємо старі записи по цій монеті, щоб записати актуальні зі статусами
                            session.query(OrderBlock).filter_by(symbol=sym).delete()
                            
                            # Функція для розрахунку статусу
                            def add_ob(o_list, type_str):
                                for b in o_list:
                                    ...
                                    # Якщо ціна близько (0.5%)
                                    elif abs(curr_price - entry)/entry < 0.005: status = '⚠️ NEAR'
                                    
                                    session.add(OrderBlock(
                                        symbol=sym, timeframe=str(htf), ob_type=type_str,
                                        top=b['top'], bottom=b['bottom'], entry_price=entry, sl_price=sl,
                                        created_at=b['created_at'], status=status
                                    ))
                    ...
                    time.sleep(1.0) # Пауза між монетами
                session.close()
                time.sleep(15) # Пауза після проходу всього списку
