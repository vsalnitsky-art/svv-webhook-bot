"""
Statistics Service - Full V3
"""
from models import db_manager, WhaleSignal, CoinStatistics, Trade, CoinPerformance, TradeMonitorLog
...
class StatisticsService:
    ...
    def save_whale_signal(self, d):
        ...
        try:
            ...
            s.add(w); s.commit()
            self._update_coin_statistics(s, d['symbol'])
        except: pass
        finally: s.close()
    ...
    def _update_coin_statistics(self, s, sym):
        try:
            st = s.query(CoinStatistics).filter_by(symbol=sym).first()
            if not st: st = CoinStatistics(symbol=sym, first_seen=datetime.utcnow()); s.add(st)
            st.last_updated = datetime.utcnow()
            s.commit()
        except: pass

    def delete_coin_history(self, sym):
        # Видаляємо сигнали, але НЕ видаляємо TradeMonitorLog, щоб бачити історію зліва
        s = self.db.get_session()
        try:
            s.query(WhaleSignal).filter_by(symbol=sym).delete()
            s.commit()
        except: pass
        finally: s.close()
    ...
    def cleanup_old_data(self, days=7):
        ...
        try:
            ...
            s.query(TradeMonitorLog).filter(TradeMonitorLog.timestamp < c).delete()
            s.commit()
        except: pass
        finally: s.close()
