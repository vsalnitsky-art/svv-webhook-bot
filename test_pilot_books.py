...
def test_level_fields_keep_trailing_zeros():
    ...
    # ⚠️ ЗАМОК ПЕРЕПИСАНО (18.09), а не «полагоджено»: `lang="en"` на
    # `<input type=number>` НЕ спрацював — на скріні поля показували
    # `0,38000` / `136,00` поруч із `$0.38000` у колонці автопілота.
    # Firefox цей атрибут для числових полів ігнорує взагалі. Тепер поле —
    # `type="text" inputmode="decimal"`, тобто значення малюємо МИ.
    j = html.index('function manualSlTpCellsHTML')
    cells = html[j:j + 2600]
    # ⚠️ Рядки-коментарі прибираємо: замок падав на ВЛАСНОМУ поясненні, де
    # `type="number"` згадано як те, чого більше НЕ робимо (та сама пастка,
    # що вже ловилась на `ensure_fresh`).
    code = '\n'.join(l for l in cells.splitlines()
                     if not l.lstrip().startswith('//'))
    _check('type="number"' not in code,
           'числове поле знову віддає формат локалі браузера')
    _check('inputmode="decimal"' in cells and 'type="text"' in cells,
           'поля рівнів мусять малюватись нами, а не браузером')
