...
def plan_targets(side: str, entry, price, targets: List[Dict],
    ...
    # число із 🎯 Автопілот; у випадку якщо число 🎯 Автопілот менше за
    # автоматичне значення Manual TP-2, то поле Manual TP-1 залишаємо порожнім».
    #
    # «Менше» тут — НЕ про арифметику, а про НАПРЯМОК: рівень мусить лежати
    # МІЖ входом і TP-2. Для LONG це «нижче за TP-2», для SHORT — «вище».
    # Саме тому порівняння йде через `_ahead`, а не через `<`: буквальне `<`
    # на шорті означало б протилежне і ставило б частковий вихід ДАЛІ за повний.
    #
    # ⚠️ Коли TP-2 прийшов із 🧲 МАГНІТУ, власна ціль автопілота (найдальший
    # обʼєкт графіка) — це ОКРЕМЕ, змістовне число, і саме воно стає TP-1.
    # Пошук «найсильнішого обʼєкта у вікні шляху» лишається ЛИШЕ як запасний
    # шлях: він потрібен, коли магніту немає і власне число автопілота ЗБІГЛОСЬ
    # із TP-2 (інакше TP-1 не було б де взяти взагалі).
    # ⚠️ Не підійшло власне число → **порожньо**, БЕЗ похідного рівня: користувач
    # просив саме лишати поле порожнім, а не вигадувати заміну.
    tp1 = None
    _own = pick_objective(targets or [])
    _own_lvl = _f((_own or {}).get('price'))
    _own_decides = _own_lvl is not None and abs(_own_lvl - tp2['price']) > 1e-12

    if _own_decides:
        _c = _pack(_own)
        if not _ahead(side, e, _own_lvl):
            reasons.append(f"TP-1 порожній: число 🎯 Автопілота {_own_lvl:.8g} "
                           f"позаду входу")
        elif not _ahead(side, _own_lvl, tp2['price']):
            reasons.append(f"TP-1 порожній: число 🎯 Автопілота {_own_lvl:.8g} "
                           f"не перед TP-2 {tp2['price']:.8g} "
                           f"(рівень не вигадуємо)")
        elif _c['from_entry_pct'] < gap:
            reasons.append(f"TP-1 порожній: число 🎯 Автопілота за "
                           f"{_c['from_entry_pct']:.2f}% від входу — ближче за "
                           f"зазор {gap:g}% (комісія зʼїла б частковий вихід)")
        elif abs(tp2['price'] - _own_lvl) / _own_lvl * 100.0 < gap:
            reasons.append(f"TP-1 порожній: число 🎯 Автопілота впритул до TP-2 "
                           f"(зазор < {gap:g}%)")
        else:
            _c['path_pct'] = round(_c['from_entry_pct'] / span * 100.0, 1) if span > 0 else 0.0
            tp1 = _c
            reasons.append(f"TP-1 (частковий) = ціль 🎯 Автопілота: {_c['label']} "
                           f"@ {_c['price']:.8g} (+{_c['from_entry_pct']:.2f}% "
                           f"від входу, {_c['path_pct']:.0f}% шляху до TP-2"
                           + (f", {_c['r']}R)" if _c['r'] else ')'))
        return _finish_targets(side, tp1, tp2, reasons)

    # ═══ ЗАПАСНИЙ ШЛЯХ: магніту немає, власне число = TP-2 ════════════════
    # Тоді TP-1 шукаємо серед обʼєктів графіка у ВІКНІ ЧАСТКИ ШЛЯХУ.
    cands = []
    for t in (targets or []):
        lvl = _f(t.get('price'))
...
def _finish_targets(side, tp1, tp2, reasons) -> Dict:
    """ОСТАННІЙ ЗАМОК: TP-1 НІКОЛИ не перевищує TP-2.

    ⚠️ Вимога користувача дослівно: «Manual TP-1 не має перевищувати Manual
    TP-2. Якщо так трапляється, TP-1 залишити пустим (не вірно визначився
    Автопілот)». УСІ гілки вище перевіряють це самі — але правило надто
    дороге, щоб покладатись на те, що майбутня правка однієї з них його не
    загубить. Тепер виходів ДВА (власне число автопілота і запасний пошук),
    тож замок винесено в спільну функцію, щоб його не можна було обійти
    новою гілкою.
    Порожній TP-1 — КОРЕКТНИЙ стан (працюємо одним TP-2); TP-1 ЗА межею
    TP-2 — ні: «частковий» вихід опинився б ДАЛІ за повний.
    """
    if tp1 is not None and not _ahead(side, tp1['price'], tp2['price']):
        reasons.append(f"⚠️ TP-1 {tp1['price']:.8g} не перед TP-2 "
                       f"{tp2['price']:.8g} — рівень НЕ виставляємо "
                       f"(лишається порожнім)")
        tp1 = None
