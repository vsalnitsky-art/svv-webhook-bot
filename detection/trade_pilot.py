...
def plan_targets(side: str, entry, price, targets: List[Dict],
    ...
    # Автопілот)». Обидві гілки вище вже перевіряють це самі — але правило
    # надто дороге, щоб покладатись на те, що майбутня правка однієї з гілок
    # його не загубить. Порожній TP-1 — коректний стан (працюємо одним TP-2);
    # TP-1 ЗА межею TP-2 — ні: «частковий» вихід опинився б ДАЛІ за повний.
