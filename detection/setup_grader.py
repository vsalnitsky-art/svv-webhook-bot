...
# ── ЄДИНЕ ДЖЕРЕЛО ПРАВДИ для порогів «якості» ───────────────────────────────
# Ці ж межі мусять використовувати ВСІ споживачі: випадайки в UI, шари Черги-4,
# шаровий конфлюенс funding тощо. РАНІШЕ вони дублювались числами в HTML і в
# fuel_filter (СЕРЕДНІЙ був 40 у випадайці, але 38 у грейді) → на екрані
# «38% СЕРЕДНІЙ» НЕ проходив поріг «≥ СЕРЕДНІЙ». Дублювати числа ЗАБОРОНЕНО —
# брати через grade_bands()/grade_min().
def grade_bands():
    """[{'min': поріг, 'label': назва, 'color': колір}] — від найвищого донизу
    (без службового 'ВИЧЕРПАНО'). Для побудови випадайок в UI."""
    return [{'min': m, 'label': lbl, 'color': c}
            for (m, lbl, c) in _GRADE if m > 0]


def grade_min(label, default=0):
    """Мінімальний бал для назви грейда ('СЕРЕДНІЙ' → 38). Єдине джерело —
    таблиця _GRADE, тому назва і число НІКОЛИ не розходяться."""
    _l = str(label or '').strip().upper()
    for (m, lbl, _c) in _GRADE:
        if lbl == _l:
            return m
    return default
