...
Тому тут:
  ...
  • **КІНЕЦЬ** — вирішує ЛИШЕ ширина (`mm_corr_breadth_exit`): поки частка
    монет ПРОТИ банера ≥ `mm_corr_vob_exit_pct` (деф. **65%**),
...
DEFAULTS = {
    ...
    # може завершитись.
    # ⚠️ 50 → 65 ЗА РІШЕННЯМ КОРИСТУВАЧА (22.09): «хочу 📦 ≥60 на старт і
    # кінець <65». БІЛЬШЕ число = корекція завершується РАНІШЕ, тобто це
    # МʼЯКША настройка, ніж 50.
    # ⚠️ Вихід тепер МОЖЕ бути вищим за вхід, і це НЕ поломка — див.
    # `breadth_exit_ok`. Прогін на справжніх функціях: при ширині 90% (кейс
    # скарги 22.09) 65 поводиться ТОЧНО як 50 — жодного хибного кінця; різниця
    # лише у вузькій смузі [вхід..вихід), де вето не діє.
    'mm_corr_vob_exit_pct': 65.0,
...
def breadth_exit_ok(vob: Optional[Dict], exit_pct) -> Optional[bool]:
    ...
    ⚠️ **ВИХІД МОЖЕ БУТИ ВИЩИМ ЗА ВХІД** (рішення користувача 22.09; раніше
    валідація це зводила). Наслідок рівно один і він ЛОКАЛЬНИЙ: у смузі
    [вхід .. вихід) вето не діє, і кінець там знову вирішують швидкі шари —
    тобто стара поведінка «голосів» У ЦІЙ СМУЗІ. ВИЩЕ за поріг виходу захист
    працює як раніше. Заміряно прогоном через `next_state`: при ширині 90%
    (кейс скарги) 65 і 50 дають ІДЕНТИЧНИЙ результат — один старт, жодного
    хибного кінця; різниця зʼявляється лише коли ширина зависає в самій смузі.
    """
    v = dict(vob or {})
...
def evaluate(snap: Dict, trends: Dict, bias: str, lever_now, lever_peak,
    ...
    exit_pct = _num(c['mm_corr_vob_exit_pct'], 65.0)
