...
class FVGDetector:
    ...
    def __init__(
        ...
        zl_service=None,  # External ZeroLagTrendService reference
        ...
        self._zl_service = zl_service  # Shared ZLT service
    ...
    def _check_trend_filter(self, symbol: str, direction: str) -> tuple:
        ...
        Checks: EMA LTF → EMA HTF → Zero Lag Trend service (if connected).
        """
        # EMA LTF check
        ...
        # Zero Lag Trend check (external service)
        if self._zl_service:
    ...
    def _scan_symbol(self, symbol: str) -> int:
        ...
        try:
            ...
            # Zero Lag Trend: provide 15m klines to service (avoid duplicate REST)
            if self._zl_service and self._zl_service.enabled and self.timeframe == '15m':
                self._zl_service.update_symbol(symbol, klines_15m=klines)
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self._stats,
            ...
            'zl_length': self._zl_service.length if self._zl_service else 70,
            'zl_mult': self._zl_service.mult if self._zl_service else 1.2,
            'zl_trends': self._zl_service._trends.copy() if self._zl_service else {},
        }
