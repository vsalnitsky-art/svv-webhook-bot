...
Ключові рішення:
  • **ДЕДУП ЗА ІМЕНЕМ.** Другий запит того самого скану, поки перший ще чекає,
    НЕ стає в чергу окремо — він чекає на ТОЙ САМИЙ результат. Інакше
    нетерплячий подвійний клік по «Сканувати» коштував би подвійного залпу,
    а дав би ті самі числа.
...
class ScanQueue:
    ...
    def submit(self, name: str, fn: Callable, source: str = '') -> _Job:
        """Поставити скан у чергу. Той самий `name`, що ВЖЕ чекає, другим
        завданням не стає — повертається той самий job (див. дедуп вище)."""
        with self._cv:
            for j in self._q:
                if j.name == name:
                    return j
