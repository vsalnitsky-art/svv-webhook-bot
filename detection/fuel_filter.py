...
class FuelFilterDaemon:
    ...
    def _mm_track_bias(self, snap: Dict, now: float,
        ...
        **Вимога користувача (17.09), дослівно:** «За основу банера візьми
        ...
        ⚠️ **Монети В УГОДІ не рахуються.** Важіль описує ВІЛЬНИЙ ринок —
        те, куди ще можна зайти (саме цей напрямок бере 💧 Сканер). З 18.09
        такі монети в таблиці ВИДНО (колонка «📍 Стан»), тож розбіжність
        «рядків більше, ніж у важелі» названа вголос у підказці банера —
        інакше вона читалась би як помилка підрахунку.
