...
DEFAULT_SETTINGS = {
    'enabled': False,
    # ── Активна стратегія ────────────────────────────────────────────────
    # 'readiness' — «Готовність» (ДЕФОЛТ/активна): двигун відкриває монету з
    #               черги, щойно її SCORE = STRONG HOLD і напрямок SCORE
    #               збігається з кнопкою, БЕЗ ₿ START / сеансів ММ. Логується.
    # 'fuel'      — уся стара логіка (сеанси ММ + ₿ START + паливо), доступна
    #               через селектор у будь-який момент.
    'active_strategy': 'readiness',
    'readiness_log_enabled': True,   # писати рішення «Готовності» в БД + UI-лог
...
class FuelFilterDaemon:
    def __init__(self, db, get_trade_manager: Callable,
        ...
        # «Готовність» strategy: anti-flood for the per-coin decision log.
        # {symbol: (outcome, label, ts)} — a hold/skip is re-logged only when
        # its (outcome,label) changes or after READINESS_LOG_MIN_GAP seconds;
    ...
    def get_settings(self) -> Dict:
        ...
        # Active strategy — only two valid values; anything else → 'fuel'.
        if s.get('active_strategy') not in ('fuel', 'readiness'):
            s['active_strategy'] = 'fuel'
    ...
    def _timer_score_for(self, symbol, direction, held_sec, exhaustion,
        ...
        return {'score': int(round(score)), 'label': label, 'color': color,
                ...
                # Raw SCORE components (0..1) — captured for «Готовність»
                # per-decision logging / analysis.
                'components': {'room': round(room, 4), 'hold': round(hold, 4),
                               'fuel': round(fmag, 4), 'mom': round(mom, 4)}}
    ...
    def _engine_tick(self):
        s = self.get_settings()
        # STRATEGY SWITCH: «Готовність» runs a completely separate open path
        # (SCORE=STRONG HOLD, no ₿ START/session gating). The fuel strategy
        # below is left untouched and only runs when it is the active one.
        if (s.get('active_strategy') or 'fuel') == 'readiness':
            self._engine_tick_readiness(s)
            return
    ...
    # Re-log an unchanged hold/skip decision at most this often (seconds).
    READINESS_LOG_MIN_GAP = 300

    def _log_readiness(self, symbol: str, signal_dir: str, sc: Optional[Dict],
                       outcome: str, reason: str, enabled: bool):
        """Persist ONE «Готовність» decision to the readiness_log table and a
        concise line to the shared event log (UI). Hold/skip decisions are
        throttled so an idle queue doesn't flood the table; 'opened' always
        logs. Best-effort — never raises into the engine loop."""
        if not enabled:
            return
        label = (sc or {}).get('label')
        # Throttle unchanged hold/skip; always log an actual open.
        if outcome != 'opened':
            prev = self._readiness_last_log.get(symbol)
            now = time.time()
            if prev and prev[0] == outcome and prev[1] == label \
                    and (now - prev[2]) < self.READINESS_LOG_MIN_GAP:
                return
            self._readiness_last_log[symbol] = (outcome, label, now)
        else:
            self._readiness_last_log.pop(symbol, None)
        comp = (sc or {}).get('components') or {}
        try:
            self._db.log_readiness(
                symbol=symbol, signal_dir=signal_dir,
                score=(sc or {}).get('score'), label=label,
                score_dir=(sc or {}).get('dir'),
                room=comp.get('room'), hold=comp.get('hold'),
                fuel=comp.get('fuel'), momentum=comp.get('mom'),
                fuel_strength=(sc or {}).get('fuel_strength'),
                exhaustion=(sc or {}).get('exh'),
                conflict=bool((sc or {}).get('conflict')),
                outcome=outcome, reason=reason)
        except Exception as e:
            print(f"[FF-Readiness] readiness_log error {symbol}: {e}")
        # Concise line into the shared event log (dashboard UI stream).
        try:
            icon = {'opened': '✅', 'hold': '⏳', 'skipped': '⛔'}.get(outcome, '•')
            lvl = 'SUCCESS' if outcome == 'opened' else 'INFO'
            sctxt = (sc or {}).get('score')
            self._db.log_event(
                f"🎯 Готовність {icon} {symbol} {signal_dir} · "
                f"SCORE {sctxt if sctxt is not None else '—'} "
                f"{label or '—'} · {reason}",
                level=lvl, category='READINESS', symbol=symbol)
        except Exception:
            pass

    def _engine_tick_readiness(self, s: Dict):
        """«Готовність» open engine. Candidates come from the SAME intercepted
        queue (_pending) and the SAME LONG/SHORT buttons, but a coin opens the
        moment its per-coin SCORE is STRONG HOLD in the queued direction — NO
        ₿ START, NO session gating. Every evaluation is logged for analysis."""
        ...
        with self._lock:
            for sym, info in self._pending.items():
                ...
                cand[sym] = (now - info.get('added_at', now), d)
        if not cand:
            print(f"[FF-Readiness] 0 кандидатів (кнопки L={allow_long} S={allow_short})")
            return
        ...
        # Open oldest-waiting first (same fairness as the fuel engine).
        for sym, (held, d) in sorted(cand.items(), key=lambda kv: -kv[1][0]):
            if sym in self._fuel_managed:
                trace.append(f"{sym}:managed")
                continue
            # Skip / drop if TM already holds this coin (real OR paper).
            if self._tm_has_position(sym, True) or self._tm_has_position(sym, False):
                trace.append(f"{sym}:вже-в-угодах")
                if _q_allowed(3):   # OP 3: TM already holds the coin
                    with self._lock:
            ...
            exh = self._exhaustion(sym, d)
            sc = self._timer_score_for(sym, d, held, exh, dur, tf)
            # Exhaustion gate — same ceiling as the fuel engine.
            if exh is not None and exh > max_exh:
                self._log_readiness(sym, d, sc, 'skipped',
                                    f"виснаж {exh:.0f}%>{max_exh:.0f}%", log_on)
                trace.append(f"{sym}:виснаж{exh:.0f}%")
                continue
            # READINESS trigger: SCORE must be STRONG HOLD AND its live
            # direction must match the queued signal direction.
            if sc.get('label') != 'STRONG HOLD' or sc.get('dir') != d:
                self._log_readiness(sym, d, sc, 'hold',
                                    f"score={sc.get('label')}/{sc.get('dir')}≠STRONG·{d}",
                                    log_on)
                trace.append(f"{sym}:{sc.get('label')}/{sc.get('dir')}")
                continue
            try:
                opened = self._open(
                    sym, d, fuel, s,
                    opened_by=f"🎯 Готовність SCORE {sc.get('score')}")
                if opened:
                    with self._lock:
                        self._timers[sym] = {'dir': d, 'since': now,
                                             'start_price': mark}
                        if _q_allowed(2):   # OP 2: engine opened the trade
                            self._pending.pop(sym, None)
                    self._log_readiness(sym, d, sc, 'opened',
                                        f"STRONG HOLD SCORE {sc.get('score')}", log_on)
                    trace.append(f"{sym}:✅ВІДКРИТО {d}")
                    print(f"[FF-Readiness] opened {d} {sym} (SCORE {sc.get('score')})")
                else:
                    self._log_readiness(sym, d, sc, 'skipped', '_open-відхилив', log_on)
                    trace.append(f"{sym}:_open-відхилив")
            except Exception as e:
                trace.append(f"{sym}:помилка")
        ...
        print(f"[FF-Readiness] кнопки L={allow_long} S={allow_short} · "
              f"{len(cand)} канд · SCORE-гейт · " + ' '.join(trace))
