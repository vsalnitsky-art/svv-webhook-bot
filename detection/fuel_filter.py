...
# 15хв або на 5хв». Тобто обране джерело — ПЕРШЕ, а далі спускаємось по
# МОЛОДШИХ Volumized-блоках, поки не знайдеться придатний рівень.
# ⚠️ Порядок СПАДНИЙ (старший → молодший): чим молодший TF, тим ближче блок до
# ціни, отже тим ТІСНІШИЙ стоп. Ставити 5m попереду 15m означало б системно
# звужувати ризик без рішення користувача.
# ⚠️ ОДИН список на ОБИДВА шляхи авто-SL (`fuel_filter._q4_sl_candidates` і
# `trade_manager._auto_ob_manual_sl`) — два «однакових» ланцюги розійшлися б, і
...
DEFAULT_SETTINGS = {
    ...
    #   Новий МММ (сила ≥ queue4_mm_new_min), МММ LiQ (≥ queue4_mm_old_min),
    ...
    'queue4_old_mm_on': True,    # МММ LiQ (у воротах + Складність)
    ...
    'queue4_mm_old_min': 10,     # МММ LiQ: ті самі band-и
    # 🧭 Режим шару «МММ LiQ» у Черзі-4. МММ LiQ = контраріанський магніт
    ...
    #    'contra_ok' — зараховувати, ЯКЩО МММ LiQ НЕ сильно ПРОТИ напрямку.
    ...
    #    в бік сигналу (сирий напрямок Новий/МММ LiQ/Готовність/Запас, БЕЗ
...
class FuelFilterDaemon:
    def __init__(self, db, get_trade_manager: Callable,
        ...
        #   обидва напрямки; відкриває, коли 4 шари (Новий/МММ LiQ,
    ...
    def _fuel_dir_legacy(self, symbol: str) -> Optional[Dict]:
        """Показник МММ LiQ: сирий (fa−fb)/den по liq-map (розташування кластерів
    ...
    def _mm_track_bias(self, snap: Dict, now: float,
        ...
        **Вимога користувача (17.09), дослівно:** «За основу банера візьми
        ...
        банер і колонка «🧮 МММ LiQ» не можуть показати різні числа
        (той самий урок, що з шарами Черги-4: двигун рахує — стан читає).
        ⚠️ **РАХУЮТЬСЯ ВСІ МОНЕТИ МОНІТОРА, у тому числі ті, що В РОБОТІ**
        (вимога 18.09, дослівно: «за показник банера "МММ-монітор" мають
        відповідати всі монети, для цього і повернули щоб всі монети були у
        таблиці»). Раніше монети у відкритій угоді з важеля ВИКИДАЛИСЬ — це
        трималось на старому правилі «таблиця показує лише кандидатів». Правило
        скасовано, тож і виняток пішов: банер знову описує РІВНО те, що під ним
        видно, і його число сходиться з рядками таблиці.
    ...
    def _mm_queue_map(self) -> Dict[str, List[str]]:
        ...
        Потрібен колонці «📍 Стан» монітора (вимога 18.09: «пиши де на даний
        момент монета»). Черги перебираються ТУТ і лише тут; `symbols_in_work`
        (для 💧 Сканера) відповідає на ІНШЕ питання — «чи зайнята взагалі» — і
        свідомо лишається окремим, простішим набором.
        """
        out: Dict[str, List[str]] = {}
        with self._lock:
            for lbl, d in (('Q1', self._pending), ('Q2', self._pending2),
                           ('Q3', self._pending3), ('Q4', self._pending4)):
                for k in (d or {}):
    ...
    def mm_monitor_state(self, settings: Optional[Dict] = None) -> Dict:
        ...
        ЗНІМКА такі монети НЕ прибираються: він живить колонку «🧮 МММ LiQ»
        ...
        for sym, v in snap.items():
            ...
            rows.append({
                ...
                # монета В УГОДІ теж не обирається (друга позиція по тому
                # самому символу все одно не відкриється).
                'selectable': st in ('LONG', 'SHORT') and not _trade,
            })
        ...
        return {
            ...
            #   no_data  — узяв, але МММ LiQ по них ще не порахувався
            #              (liq-map не зібрала рівнів / монета щойно додана);
            #   in_trade — скільки з ПОКАЗАНИХ рядків уже В УГОДІ.
            # ⚠️ З 18.09 монети в угоді з таблиці НЕ зникають (вимога), тож
            # розклад сходиться так: `targeted − no_data == rows`, а `in_trade`
            # — це підмножина рядків, а не «загублені» монети.
            'coverage': {
                'targeted': int(_cov.get('targeted') or 0),
                'no_data': max(0, int(_cov.get('targeted') or 0)
                               - int(_cov.get('data') or 0)),
                'in_trade': int(_in_trade),
                'rows': len(rows),
            } if _on else {},
    ...
    def mm_snapshot_for(self, symbols) -> Dict:
        ...
        Живить колонку «🧮 МММ LiQ» у таблицях ВІДКРИТИХ УГОД. Сюди
    ...
    def _mm_only_tick(self, settings: Dict):
        ...
        **Скарга 18.09:** «МММ-монітор взагалі перестав працювати. Навіть у
        ...
        черг стосунку не мають:
          ...
          • колонку «🧮 МММ LiQ» у таблицях ВІДКРИТИХ УГОД;
    ...
    def _tick(self):
        ...
        if not settings.get('enabled'):
            ...
            # «🧮 МММ LiQ» в угодах і ПРАВИЛО ВИХОДУ `use_mm_flat_exit`.
    ...
    def _q4_snapshot(self, sym: str, s: Dict) -> Dict:
        ...
        # 2) МММ LiQ — окремий показник; читаємо лише коли шар увімкнено.
        fo = {}
        if on_old:
    ...
    def _q4_layers_from(self, snap: Dict, side: str, s: Dict) -> Dict:
        ...
        напрямком (усі зелені LONG / усі червоні SHORT). Шари:
          ...
          2) МММ LiQ — _fuel_dir_legacy:  напрямок у бік + сила ≥ queue4_mm_old_min
        ...
        # 2) МММ LiQ — зі ЗНІМКА (legacy читається там лише коли шар увімкнено).
        od = None; os_ = 0
        if on_old:
            ...
            add('mm_old', 'МММ LiQ', _old_ok, _old_detail,
    ...
    def _q4_sl_candidates(self, sym: str, side: str, src: str):
        """Джерела рівня SL у порядку пріоритету: обране користувачем → молодші.

        🛑 СХОДИ (вимога 22.09): обране «1H OB» → **15m Volumized → 5m
        Volumized**. Раніше фолбек був ОДИН (15m), тож коли й він не давав
        блоку, Черга-4 лишала угоду без стопа і вся надія була на гарантію
        «% від входу» в TM. Перелік молодших TF — спільний `SL_FALLBACK_TFS`.
        Обрано «15m Volumized» → 15m → 5m → 1H OB (та сама драбина, просто
        починається нижче).
        ...
        def _from_vob(tf: str):
            # ⚠️ Фабрика, а не лямбда в циклі: інакше пізнє звʼязування дало б
            # УСІМ кандидатам останній TF списку.
            def _g():
                r = self._q4_ob_bounds_vob(sym, side, tf)
                if not r:
                    return None, f'{tf} Volumized OB: немає блоку'
                top, bottom, _tf = r
                return (top, bottom, f'Volumized OB ({_tf})'), None
            return _g

        _vob = [_from_vob(tf) for tf in SL_FALLBACK_TFS]
        if src == '1h':
            return [_from_1h] + _vob
        # Обрано молодший блок — 1H стає ОСТАННІМ фолбеком, а не зникає.
        return _vob + [_from_1h]
    ...
    def _q4_set_vob_sl(self, sym: str, side: str, s: Dict):
        ...
        Джерело блоку обирає користувач — `queue4_sl_source` («🛑 SL з»):
          • '1h' (ДЕФОЛТ) — 1H Order Block (★-блок сканера на `ob_filter_timeframe`),
                            далі фолбек 15m Volumized → 5m Volumized;
          • '15m'         — Volumized OB на 15m, далі 5m Volumized → 1H OB.
...
def init_fuel_filter(db, get_trade_manager, get_watchlist) -> FuelFilterDaemon:
    ...
    «МММ LiQ» в угодах і НЕПРАЦЮЮЧЕ правило виходу `use_mm_flat_exit`.
