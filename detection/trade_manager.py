...
DEFAULT_SETTINGS = {
    ...
    # Скільки СЕКУНД мусить протриматись РОЗВОРОТ МММ ПРОТИ угоди (режим
    # 'flat_or_against'), перш ніж закривати. Вимога 20.09 дослівно: «потрібно
    # трішки витримки, бо секундна зміна і вибиває з угоди».
    # ⚠️ ОКРЕМЕ поле, а не спільне з ⚖ рівновагою: це різні події (там «тиску
    # немає», тут «тиск ПРОТИ»), і ціна очікування в них різна.
    # Дефолт 60с ≈ одне оновлення liq-map: розворот мусить пережити хоча б один
    # НОВИЙ розрахунок джерела, а не миттєве тремтіння ціни. 0 = одразу.
    'mm_against_exit_confirm_sec': 60,
...
class TradeManager:
    ...
    def __init__(self, db=None, notifier=None, bybit=None, scanner=None):
        ...
        self._mm_against_since: Dict[str, float] = {}
    ...
    def update_settings(self, new: Dict) -> Dict:
        with self._lock:
            ...
            try:
                self._settings['mm_against_exit_confirm_sec'] = max(
                    0, int(float(self._settings.get('mm_against_exit_confirm_sec', 60) or 0)))
            except (TypeError, ValueError):
                self._settings['mm_against_exit_confirm_sec'] = 60
    ...
    def _mm_flat_exit_reason(self, symbol: str, pos: Dict) -> Optional[tuple]:
        ...
        ⚠️ **ДВІ ВИТРИМКИ, КОЖНА СВОЯ** (вимога 20.09 — див. нижче): ⚖ рівновага
        тримає `mm_flat_exit_confirm_sec`, розворот ПРОТИ —
        `mm_against_exit_confirm_sec`. Одне поле на обидві події не годиться:
        вони різні за природою і за ціною помилки.
        """
        s = self._settings
        ...
        if not hit:
            # Напрямок тримає угоду → ОБИДВА таймери підтвердження ОБНУЛЯЄМО,
            # інакше короткий провал «накопичувався» б між епізодами.
            self._mm_flat_since.pop(sym, None)
            self._mm_against_since.pop(sym, None)
            return None
        now = time.time()
        ...
        # • ⚖ РІВНОВАГА — витримка проти МИГОТІННЯ біля межі |dir| ≤ 0.1: там
        #   показник стрибає між «рівновага» і «слабкий напрямок».
        # • 🔄 РОЗВОРОТ ПРОТИ — витримка проти ОДНОГО ТАКТУ зустрічного тиску
        #   (вимога 20.09, дослівно: «потрібно трішки витримки, бо секундна
        #   зміна і вибиває з угоди»). ⚠️ ЦЕ СКАСОВУЄ правило 18.09
        #   («коли "проти" то закривати відразу») — вимога змінилась, і замок
        #   переписано, а не «полагоджено».
        # Спільним полем їх робити НЕ МОЖНА: рівновага це «тиску немає»
        # (чекати дешево), а розворот це «тиск ПРОТИ» (чекати дорого), тож
        # числа тут різні за змістом і за ціною помилки.
        if against:
            # Таймер рівноваги знімаємо: він міряє САМЕ ⚖, і лишати його «в
            # спадок» розвороту не можна (і навпаки — нижче).
            self._mm_flat_since.pop(sym, None)
            a_since = self._mm_against_since.setdefault(sym, now)
            a_need = float(s.get('mm_against_exit_confirm_sec') or 0)
            a_held = now - float(a_since)
            if a_need > 0 and a_held < a_need:
                # Тримаємо угоду й ЧЕКАЄМО: розворот мусить протриматись. Якщо
                # МММ повернеться — таймер зніме гілка «напрямок тримає».
                return None
            _ah = f' · тримається {int(a_held)}с' if a_need > 0 else ''
            # ⚠️ ОКРЕМИЙ КОД ПРИЧИНИ (скарга 18.09: «монета закрилась саме по
            # протилежному значенню, а не рівновага — потрібно точно розрізнити
            # причини закриття»). Раніше обидві гілки віддавали `mm_flat_exit`,
    ...
    def _pilot_tick(self, symbol: str, pos: Dict, current_price: float,
        ...
        # Вихід із угоди тоді веде ОДИН господар — 🧮 показник; автопілот не має
        # ні закривати на своїй цілі, ні вести стоп, ні підставляти свої рівні.
        # ⚠️ ОДИН гейт на ВЕСЬ такт, а не сім окремих: усе автоматичне
        # (вибір цілі, 🧲 магніт, трейл, `take`, автозаповнення TP, якір R)
        # живе САМЕ тут, тож вимкнути його можна рівно в одному місці.
        ...
        try:
            ...
            _prog = None
        # 📐 R угоди = (вхід→ціль) / (вхід→ПОЧАТКОВИЙ стоп).
        #
        # 🐞 КЕЙС VIRTUALUSDT (05.09) — «168.53R». Число було арифметично
        # ПРАВИЛЬНЕ і при цьому беззмістовне: вхід 0.67160, ціль 0.72890, а
        # стоп після спрацювання TP-1 переїхав у БЕЗЗБИТОК (0.67194, тобто на
        # 0.05% ВИЩЕ входу). Знаменник став 0.00034 → R вибухнув.
        # У таблиці це читається як «геніальний сетап», хоча насправді
        # означає протилежне: ризику вже НЕМАЄ, угода веде хвіст.
        #
        # **R міряємо від ПОЧАТКОВОГО стопа** — це і є канонічне значення R
        # («1R = скільки я ризикнув на вході»). Якір `pilot_r_stop`
        # ставиться ОДИН раз, щойно в угоді зʼявився стоп, і НЕ переписується
        # ні трейлом, ні беззбитком, ні рукою оператора — інакше показник
        # знову «поплив би» разом зі стопом.
        # ⚠️ Стоп ще не стоїть → якоря немає → R немає (не вигадуємо).
        _sl_now = pos.get('manual_sl')
        try:
            _rfree = trade_pilot.is_risk_free(side, pos.get('entry_price'), _sl_now)
        except Exception:
            _rfree = False
        # ⚠️ Якір ставимо ЛИШЕ на стоп, у якому ще Є ризик. Угода, відкрита ДО
        # появи цього якоря, приходить сюди вже з беззбитковим стопом — і
        # записати ЙОГО як «початковий» означало б зберегти той самий
        # роздутий R назавжди. Початковий ризик відновити нізвідки: чесно
        # лишаємось без R (у тултипі сказано чому), а не малюємо 168.53R.
        # На новій угоді стоп при відкритті ризик-фрі бути НЕ може (TM вимагає
        # рівень із правильного боку ціни), тож якір ставиться першим же тіком.
        if not pos.get('pilot_r_stop') and _sl_now and not _rfree:
            pos['pilot_r_stop'] = _sl_now
        try:
            _rr = trade_pilot.risk_reward(pos.get('entry_price'),
                                          pos.get('pilot_r_stop'),
                                          res.get('objective'))
        except Exception:
            _rr = None
        ...
        self._pilot_state[pkey] = {
            ...
            'r': _rr,
            # Стоп, від якого рахується R (для тултипа — щоб число можна було
    ...
    def _pilot_auto_off(s: Dict) -> bool:
        ...
        ⚠️ Це НЕ вимикає сам тумблер `pilot_enabled` у налаштуваннях: вимкнувши
        🧮-правило, користувач отримує автопілот назад рівно таким, яким його
