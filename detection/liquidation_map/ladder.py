...
def fmt_band_ua(lo, hi) -> str:
    """«$76 000–78 000» — сходинка це СМУГА, а не точка.

    ⚠️ ЧОМУ ЦЕ ВАЖЛИВО (кейс BTC 05.09). Рядок підписувався ОДНІЄЮ межею
    (`price`), а відсоток поруч рахувався від СЕРЕДИНИ сходинки — тобто підпис
    і відстань вказували на РІЗНІ точки. На екрані це давало прямі
    суперечності:
      • `$76 000 · ↓3.8%` — насправді $76 000 лежить за 5.05% від ціни
        $80 038, а 3.8% — це $77 000 (середина смуги);
      • `$80 038 · ↑1.23%` — сходинку, РОЗРІЗАНУ поточною ціною, підписувало
        САМЕ ЧИСЛО ЦІНИ, тож поруч із лінією ціни стояв рядок «ціна за 1.23%
        від ціни», і зрозуміти, що це за рівень, було неможливо.
    Смуга знімає обидві: підпис описує ОБʼЄКТ (від…до), а відсоток — відстань
    до його середини, і це вже не суперечить одне одному.
    """
    a = _f(lo)
    b = _f(hi)
    if a is None:
        return ''
    if b is None or abs(b - a) < 1e-12:
        return _fmt_price_ua(a)
    hi_s = _fmt_price_ua(b)
    # Долар лишаємо ЛИШЕ на початку: «$76 000–78 000» читається як одна смуга,
    # «$76 000–$78 000» — як два окремі числа.
    if hi_s.startswith('$'):
        hi_s = hi_s[1:]
    return f"{_fmt_price_ua(a)}–{hi_s}"
...
def make_verdict(pull: str, pull_pct: float, above_pct: float,
    ...
    magnet_txt = ''
    if top_row:
        # ⚠️ СМУГА, а не одна межа: `dist` міряється від СЕРЕДИНИ сходинки,
        # тож підпис однією межею суперечив би відсотку поруч (див.
        # `fmt_band_ua`).
        _pstr = fmt_band_ua(top_row.get('price'), top_row.get('price_hi'))
