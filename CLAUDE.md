...
- **«1 VOB на 1H OB» = ТАКТ ЗА НАПРЯМКОМ 1H-OB** (`vob_one_per_ob`, дефолт OFF).
  **ТАКТ = НАПРЯМОК 1H-OB ('LONG'/'SHORT'), а НЕ `bar_time`** (рішення користувача):
  одна ФАЗА напрямку = рівно ОДИН результативний VOB. Новий 1H-OB ТОГО САМОГО
  напрямку такт НЕ поновлює — поновлює лише **ФЛІП** напрямку. Джерело —
  `_current_ob_state(sym)` → `(dir, bar_time)` одним читанням БД: `dir` = такт,
  `bar_time` потрібен ОКРЕМО лише для `_vob_before_ob` (базувати старі блоки).
  Той самий ключ у гейті відкриття `fuel_filter._ob_epoch_now` → повертає
  НАПРЯМОК. Персист: `epoch`/`fired` = рядки ('LONG'/'SHORT'), `counter` = int;
  старі int-записи ігноруються (перший скан перевизначить).
  У межах такту VOB
