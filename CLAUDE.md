...
## 🧾 VOB: у лог іде КОЖЕН блок (вимога користувача — нічого не ігнорувати)

`_vob_log_decision(...)` тепер пише в `activity_log` (`source='VOB'`) КОЖЕН стан
VOB: `numbered` / `duplicate` / `stale` / `first_sight` / `epoch` / `no_1h_ob` /
`no_candidate` — з боком, номером, віком, `ft` і деталями.
**Анти-флуд:** рядок пишеться ЛИШЕ коли стан РЕАЛЬНО змінився — ключ
`(outcome, ft, side, num)` порівнюється з попереднім записом у `_vob_diag`.
`fired`/`filtered` тут НЕ дублюються — їх уже пише основний шлях (`signal` +
`rejected` із повним розкладом фільтрів). Тест:
`test_log_decision_logs_every_vob_without_duplicates`.
⚠️ Раніше діяло протилежне правило («лог чистий, усе лише в `vob_diag`») —
користувач його СКАСУВАВ: тепер жоден VOB не має бути мовчки проігнорований.
