...
- **АУДИТ 3 значків watchlist (★ 1H-OB · ▲/▼ 5m-Volumized · ● 15m-тренд) —
  ...
  • **КОРІНЬ ▲/▼ «активна зелена / неактивна червона» (не зламати!):** фон рахував
    Volumized-тренд з `vol_data['klines']` (живі бари) через `get_latest_ob_trend`,
    а панель/намальований бокс — з `_latest_volumized_ob` (`fetch_klines(400)`) →
    РІЗНИЙ «останній OB» → у списку червоний, на панелі зелений. **Фікс:** скан
    тепер заповнює `_volumized_trend_cache['trend']` САМЕ через `_latest_volumized_ob`
    (та сама функція/klines, що й панель+бокс) → watchlist == панель == бокс для
    ВСІХ монет. Сигнали VOB лишились на `vol_result.newest_bull/bear` (окремий
    швидкий шлях на живому барі — НЕ чіпати).
