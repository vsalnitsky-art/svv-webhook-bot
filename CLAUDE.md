...
- **Гейт свіжості** (`vob_alert_max_age_bars`): **ДЕФОЛТ 0 = БЕЗ ліміту віку**
  (`_max_age=None`) → фаєримо на КОЖЕН НОВИЙ VOB (edge за `formation_time`)
  НЕЗАЛЕЖНО від того, скільки блок «висить» на графіку. Тобто **«блок на графіку =
  сигнал»** (вимога користувача — кейс ETH LONG, що «висить, а сигналу нема»).
  РАНІШЕ 0=авто(`swing+2`≈7 барів≈35хв) ТИХО різало будь-який блок, старший за
  вікно → звідси хронічне «VOB є, реакції нема». Хочеш вузьке вікно назад — постав
  `vob_alert_max_age_bars` > 0 (тоді старший за поріг → `stale`, база не рухається).
  ⚠️ Побічно: на СТАРТІ (порожній `_vob_alert_seen`) перший показ кожної монети
  тепер `fresh` → одноразовий сплеск сигналів (переважно ріжуться фільтрами).
