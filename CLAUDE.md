...
- Банер ₿ BTCUSDT = ММ-напрямок із головного вікна (compute_bias
  `components.fuel.dir`), не загальний вердикт. **Поріг той самий, що й у
  головному вікні: `> +0.1` → LONG, `< -0.1` → SHORT, `|dir| ≤ 0.1` →
  «збалансований — напрямку немає» (банер STOP, таймер скинуто).** НЕ додавати
  гістерезис/липку зону на банер — інакше він показує напрямок і лічить START,
  коли головне вікно вже пише «збалансований» (`_update_btc_verdict`).
