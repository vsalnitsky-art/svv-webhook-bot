...
- **🔎 ПРОЗОРІСТЬ VOB (жодного тихого відкидання) — вимога користувача «максимум
  інформації, чому втрачаємо сигнали».** РАНІШЕ stale/epoch/first-sight гинули
  ТИХО (без сліду) — і не було видно, ЧОМУ на панелі є валідний VOB, а сигналу
  нема. ТЕПЕР кожен НОВИЙ OB лишає слід через `_vob_log_decision(...)`:
  • **fired** — сигнал пішов (плюс звичайний `signal` рядок scanner);
  • **filtered** — відсіяно фільтром (плюс `signal`+`rejected`);
  • **stale** — новий OB, але вік > поріг → ОДИН рядок «⌛ застарілий, вік N/max»
    (раз на цей OB, анти-флуд через `_vob_diag`), + підказка підняти
    `vob_alert_max_age_bars`;
  • **epoch** — пропущено через `vob_one_per_ob` (був тихий — тепер видимий);
  • **first_sight** — базова лінія (раз на монету);
  • **duplicate/no_candidate** — стабільний стан → лише в діагностику, БЕЗ логу
    (щоб не флудити). Джерело в 🧾 Лозі: `source='VOB'`.
  Рішення edge/свіжості винесене в ЧИСТУ `_vob_edge_outcome(prev,ft,age,max_age)`
  → first_sight/duplicate/stale/fresh (юніт-тестується). Останнє рішення по
  КОЖНІЙ монеті віддається в `get_state()['vob_diag']` (для UI/інфосайту «де
  втрачаємо сигнали»). Тест: `test_vob_transparency.py`.
