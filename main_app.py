...
Updated: Added /settings page with DB storage.
"""
import logging
import threading
import time
...
def settings_page():
    ...
    <nav class="navbar bg-white mb-4 px-3 border-bottom shadow-sm">
        <div class="container">
            ...
            <div><a href="/scanner" class="btn btn-sm btn-outline-secondary me-2">Scanner</a><a href="/report" class="btn btn-sm btn-outline-secondary">Report</a></div>
        </div>
    </nav>
        ...
        <form method="POST">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-2">
                        <div class="col-md-4"><label class="form-label">Глобальний TF</label><select class="form-select" name="htfSelection"><option value="60" {{ 'selected' if conf.get('htfSelection') == 60 }}>1 Година</option><option value="240" {{ 'selected' if conf.get('htfSelection') == 240 }}>4 Години</option><option value="D" {{ 'selected' if conf.get('htfSelection') == 'D' }}>1 День</option></select></div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4"><label class="form-label">Buy Level (<=)</label><input type="number" class="form-control" name="entryRsiOversold" value="{{ conf.get('entryRsiOversold') }}"></div>
                        <div class="col-md-4"><label class="form-label">Sell Level (>=)</label><input type="number" class="form-control" name="entryRsiOverbought" value="{{ conf.get('entryRsiOverbought') }}"></div>
            ...
            <div class="row">
                <div class="col-md-6"><div class="card h-100"><div class="card-header">💰 Ризик</div><div class="card-body">
                    <div class="mb-3"><label class="form-label">Ризик (%)</label><input type="number" step="0.1" class="form-control" name="riskPercent" value="{{ conf.get('riskPercent') }}"></div>
                    <div class="mb-3"><label class="form-label">Плече (x)</label><input type="number" class="form-control" name="leverage" value="{{ conf.get('leverage') }}"></div>
                    <div class="row"><div class="col-6"><label class="form-label">ATR SL</label><input type="number" step="0.1" class="form-control" name="atrMultiplierSL" value="{{ conf.get('atrMultiplierSL') }}"></div><div class="col-6"><label class="form-label">ATR TP</label><input type="number" step="0.1" class="form-control" name="atrMultiplierTP" value="{{ conf.get('atrMultiplierTP') }}"></div></div>
                </div></div></div>
                <div class="col-md-6"><div class="card h-100"><div class="card-header">🌊 Інші</div><div class="card-body">
                    ...
                    <div class="mb-3"><label class="form-label">Swing Length</label><input type="number" class="form-control" name="swingLength" value="{{ conf.get('swingLength') }}"></div>
                </div></div></div>
            </div>
            <div class="text-center my-4"><button type="submit" class="btn btn-save btn-lg shadow">Зберегти налаштування</button></div>
        </form>
    </div>
    </body></html>
    """
...
def scanner_page():
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>body{background:#f7f9fc;font-size:14px} .text-up{color:#20b26c} .text-down{color:#ef454a}</style>
    <meta http-equiv="refresh" content="5"></head><body>
    <nav class="navbar bg-white mb-4 px-3 border-bottom shadow-sm"><div class="container-fluid"><span class="navbar-brand mb-0 h1">🐋 Active Monitor</span><div><a href="/settings" class="btn btn-sm btn-outline-primary me-2">⚙️ Налаштування</a><a href="/report" class="btn btn-sm btn-outline-secondary">Звіт</a></div></div></nav>
    <div class="container"><div class="card"><div class="card-header bg-white fw-bold py-3">ВІДКРИТІ ПОЗИЦІЇ</div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>Монета</th><th>Тип</th><th>Розмір</th><th>Вхід</th><th>RSI</th><th>Тиск</th><th>P&L (USDT)</th></tr></thead><tbody>
    {% for a in active %}<tr><td class="fw-bold">{{a.symbol}}</td><td><span class="badge {{ 'bg-success' if a.side=='Buy' else 'bg-danger' }}">{{a.side}}</span></td><td>{{a.size}}</td><td>{{a.entry}}</td><td>{{a.rsi}}</td><td>{{a.pressure}}</td><td class="{{ 'text-up' if a.pnl>0 else 'text-down' }}">{{ "+" if a.pnl > 0 }}{{a.pnl}}$</td></tr>{% else %}<tr><td colspan="7" class="text-center text-muted py-5">Немає активних угод 💤</td></tr>{% endfor %}
