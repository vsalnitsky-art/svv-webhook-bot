"""
Main Application - FINAL UKRAINIAN EDITION 🇺🇦
- Logic: Market Entry + Instant Trailing + Split TP (40%/100%)
- UI Scanner: Light Theme, Clean Monitor
- UI Report: Bybit Replica (Ukrainian), Full Charts
...
class BybitTradingBot:
    ...
    # === 🔥 ЛОГІКА: TRAILING STOP + SPLIT TP (40%/100%) ===
    def place_order(self, data):
        try:
            ...
            # 1. ВІДКРИТТЯ (MARKET)
            self.session.place_order(category="linear", symbol=norm, side=action, orderType="Market", qty=str(qty), timeInForce="GTC")
            logger.info(f"✅ OPENED: {action} {qty} {norm} @ {price}")
            
            # 2. TRAILING STOP (Активація відразу)
            ...
            self.session.set_trading_stop(
                ...
                trailingStop=str(dist), activePrice=str(price), positionIdx=0
            )
...
def scanner_page():
    ...
    <nav class="navbar navbar-light px-3"><span class="navbar-brand h6 m-0">🐋 Whale Scanner <span class="badge bg-light text-dark border">MONITOR</span></span><span class="text-muted small">{{ last_update }}</span></nav>
...
def report_page():
    ...
    # BYBIT STYLE REPORT (Повна версія)
    html = """
    <!DOCTYPE html><html lang="uk"><head><meta charset="UTF-8"><title>Аналіз P&L</title>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script><link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap" rel="stylesheet"><style>
    ...
    <div class="container">
        <div class="header"><div class="title">Аналіз P&L</div><div class="btn-group"><a href="/report?days=7" class="btn {{ 'active' if days==7 }}">7 днів</a><a href="/report?days=30" class="btn {{ 'active' if days==30 }}">30 днів</a><a href="/scanner" class="btn">← Сканер</a></div></div>
        <div class="summary-grid"><div class="stat-item"><div class="stat-label">Загальний P&L</div><div class="stat-value {{ 'text-green' if stats.total_pnl >= 0 else 'text-red' }}">{{ "+" if stats.total_pnl > 0 }}{{ "%.2f"|format(stats.total_pnl) }} USD</div></div><div class="stat-item"><div class="stat-label">Об'єм</div><div class="stat-value text-green">{{ "{:,.0f}".format(stats.total_volume) }} USD</div></div></div>
        ...
        <div class="bottom-stats"><div class="b-stat-box"><div class="b-stat-header">Всього угод</div><div class="b-stat-val">{{ stats.total_trades }}</div></div><div class="b-stat-box"><div class="b-stat-header">Win Rate</div><div class="b-stat-val">{{ stats.win_rate }} %</div></div><div class="b-stat-box"><div class="b-stat-header">P&L Long</div><div class="b-stat-val {{ 'text-green' if stats.long_pnl >= 0 else 'text-red' }}">{{ "%.2f"|format(stats.long_pnl) }}</div></div><div class="b-stat-box"><div class="b-stat-header">P&L Short</div><div class="b-stat-val {{ 'text-green' if stats.short_pnl >= 0 else 'text-red' }}">{{ "%.2f"|format(stats.short_pnl) }}</div></div></div>
        <table class="custom-table"><thead><tr><th>Контракт</th><th>Тип</th><th>P&L</th><th>Результат</th><th>Час</th></tr></thead><tbody>
        {% for t in stats.details %}<tr><td style="font-weight: 500;">{{ t.symbol }}</td><td class="{{ 'type-long' if t.side == 'Long' else 'type-short' }}">{{ "Long" if t.side == 'Long' else "Short" }}</td><td class="{{ 'text-red' if t.pnl < 0 else 'text-green' }}">{{ "+" if t.pnl > 0 }}{{ "%.4f"|format(t.pnl) }}</td><td><span class="badge {{ 'badge-success' if t.pnl > 0 else 'badge-loss' }}">{{ "Успіх" if t.pnl > 0 else "Збиток" }}</span></td><td style="color: var(--text-secondary);">{{ t.exit_time }}</td></tr>{% endfor %}
        </tbody></table>
    </div>
    <script>
