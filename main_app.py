...
Full functionality restored + OB Retest logic added properly.
"""
import logging
import threading
import time
...
def home():
    html = """
    <!DOCTYPE html><html lang="uk"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Dashboard</title>
    ...
    <div class="hero text-center"><div class="container"><h1>🤖 Trading Bot Hub</h1></div></div>
    <div class="container mb-5"><div class="row g-4">
        <div class="col-md-6 col-lg-3"><a href="/scanner"><div class="card p-4 text-center"><div class="card-body"><div class="icon">🐋</div><h4>Монітор</h4></div></div></a></div>
        <div class="col-md-6 col-lg-3"><a href="/analyzer"><div class="card p-4 text-center"><div class="card-body"><div class="icon">🚀</div><h4>Сканер</h4></div></div></a></div>
        <div class="col-md-6 col-lg-3"><a href="/settings"><div class="card p-4 text-center"><div class="card-body"><div class="icon">⚙️</div><h4>Налаштування</h4></div></div></a></div>
        <div class="col-md-6 col-lg-3"><a href="/report"><div class="card p-4 text-center"><div class="card-body"><div class="icon">📊</div><h4>Звітність</h4></div></div></a></div>
    </div></div></body></html>
    """
...
def settings_general_page():
    ...
    html = """
    <!DOCTYPE html><html lang="uk"><head><meta charset="UTF-8"><title>General Settings</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"></head><body>
    <nav class="navbar bg-white px-3 border-bottom"><div class="container-fluid"><a href="/" class="btn btn-sm btn-outline-dark">🏠 Головна</a><span class="navbar-brand mb-0 h1">⚙️ General</span><div><a href="/analyzer/settings" class="btn btn-sm btn-outline-secondary">Стратегія →</a></div></div></nav>
    <div class="container mt-4" style="max-width:700px"><form method="POST">
    <div class="card mb-3"><div class="card-header fw-bold text-primary">🌍 Глобальні</div><div class="card-body">
    <div class="mb-2"><label>Валюта</label><select class="form-select" name="scanner_quote_coin"><option value="USDT" {{ 'selected' if conf.get('scanner_quote_coin')=='USDT' }}>USDT</option><option value="USDC" {{ 'selected' if conf.get('scanner_quote_coin')=='USDC' }}>USDC</option></select></div>
    <div class="mb-2"><label>Топ монет</label><input type="number" class="form-control" name="scan_limit" value="{{ conf.get('scan_limit') }}"></div></div></div>
    <div class="card mb-3"><div class="card-header fw-bold text-danger">💰 Ризик</div><div class="card-body"><div class="row"><div class="col-6"><label>Risk %</label><input type="number" step="0.1" class="form-control" name="riskPercent" value="{{ conf.get('riskPercent') }}"></div><div class="col-6"><label>Leverage</label><input type="number" class="form-control" name="leverage" value="{{ conf.get('leverage') }}"></div></div></div></div>
    <div class="card mb-3"><div class="card-header fw-bold text-info">✈️ Telegram</div><div class="card-body"><div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" name="telegram_enabled" {{ 'checked' if conf.get('telegram_enabled') }}><label>Enabled</label></div><input type="text" class="form-control mb-2" name="telegram_bot_token" value="{{ conf.get('telegram_bot_token','') }}" placeholder="Token"><input type="text" class="form-control" name="telegram_chat_id" value="{{ conf.get('telegram_chat_id','') }}" placeholder="Chat ID"></div></div>
    <button type="submit" class="btn btn-primary w-100">Зберегти</button></form></div></body></html>
    """
...
def analyzer_settings_page():
    ...
    <nav class="navbar bg-white px-3 border-bottom"><div class="container-fluid"><a href="/" class="btn btn-sm btn-outline-dark">🏠</a><span class="navbar-brand mb-0 h1">📊 Strategy</span><div><a href="/settings" class="btn btn-sm btn-outline-secondary">← General</a></div></div></nav>
    ...
    </div><div class="row mt-2"><div class="col-6"><label>Global TF</label><select class="form-select" name="htfSelection"><option value="60" {{ 'selected' if conf.get('htfSelection')|string=='60' }}>1H</option><option value="240" {{ 'selected' if conf.get('htfSelection')|string=='240' }}>4H</option></select></div><div class="col-6"><label>Entry TF</label><select class="form-select" name="ltfSelection"><option value="5" {{ 'selected' if conf.get('ltfSelection')|string=='5' }}>5m</option><option value="15" {{ 'selected' if conf.get('ltfSelection')|string=='15' }}>15m</option></select></div></div></div></div>
...
def analyzer_page():
    ...
    <div class="container">
        <div class="accordion mb-4 shadow-sm" id="settingsAccordion"><div class="accordion-item border-0"><h2 class="accordion-header" id="headingOne"><button class="accordion-button collapsed fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSettings">🛠️ Quick Settings</button></h2>
        <div id="collapseSettings" class="accordion-collapse collapse"><div class="accordion-body bg-white"><form id="scan-form">
    ...
    <script>
    function startScan() {
        $('#btn-scan').prop('disabled', true).text('Saving...');
        var formData = {};
        ...
        $.post('/analyzer/scan', formData, function(data) { $('#btn-scan').text('Scanning...'); pollStatus(); });
    }
    function pollStatus() {
        let interval = setInterval(function() {
            $.get('/analyzer/status', function(data) {
                ...
                if (!data.is_scanning) { clearInterval(interval); location.reload(); }
            });
        }, 1000);
    }
