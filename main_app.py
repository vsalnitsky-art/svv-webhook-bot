...
def smart_money_page():
    ...
    try:
        # Показуємо тільки АКТИВНІ блоки (не пробиті), відсортовані за свіжістю
        blocks = session.query(OrderBlock).filter(OrderBlock.status != 'BROKEN').order_by(desc(OrderBlock.created_at)).all()
