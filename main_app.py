...
def ob_trend_settings_page():
    if request.method == 'POST':
        form_data = request.form.to_dict()
        # Обробка чекбоксів для стратегії (BTC REMOVED)
...
def run_scan():
    if request.form:
        ...
        for short_key, long_key in filters_map.items():
            if short_key in form_data and form_data[short_key] == 'on': form_data[long_key] = True
            else: form_data[long_key] = False
        settings.save_settings(form_data)
