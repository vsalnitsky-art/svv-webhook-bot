...
class StrategyEngine:
    ...
    def calculate_indicators(self, df):
        # Якщо даних замало, повертаємо як є (без індикаторів)
        ...
        try:
            ...
            # Розрахунок з перевіркою на помилки бібліотеки
    ...
    def check_htf_filters(self, htf_row):
        ...
        # 2. ПЕРЕВІРКА НАЯВНОСТІ КОЛОНОК (Виправлення помилки 'hma_fast')
        # Якщо індикатори не розрахувались (мало історії), пропускаємо монету
        required_cols = ['hma_fast', 'hma_slow', 'rsi', 'obv']
        for col in required_cols:
            if col not in htf_row.index:
                return {'bull': False, 'bear': False}
        
        # 3. Отримання налаштувань
        ...
        try:
            ...
            # Перевірка MFI (якщо є колонка)
            mfi_bull, mfi_bear = True, True
            if use_mfi and 'mfi_fast' in htf_row.index:
                mfi_bull = htf_row['mfi_fast'] > htf_row['mfi_slow']
                mfi_bear = htf_row['mfi_fast'] < htf_row['mfi_slow']
            ...
            # Тихий пропуск помилок розрахунку
            return {'bull': False, 'bear': False}
    ...
    def get_signal(self, df_ltf, df_htf):
        ...
        if df_ltf is None or df_htf is None: 
            return {'action': None, 'params': {}, 'reason': "", 'filters': {}}

        df_ltf = self.calculate_indicators(df_ltf)
        df_htf = self.calculate_indicators(df_htf)
        
        # Перевірка наявності колонок перед роботою
        if 'hma_fast' not in df_htf.columns or 'atr' not in df_ltf.columns:
            return {'action': None, 'params': {}, 'reason': "Not enough data", 'filters': {}}
        ...
        if filters['bull']:
            ...
            if entry_triggered:
                ...
                tp_sl = {'sl': current_price - sl_dist, 'tp': current_price + (atr * atr_tp_mult)}
